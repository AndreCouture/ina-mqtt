...
void ina219_calibrate() {
    ...
    uint16_t calib = (uint16_t)(0.04096 / (current_lsb * cfg.shunt_ohms));
    current_lsb = 0.04096 / (cfg.shunt_ohms * calib);
    power_lsb = current_lsb * 20.0;
    write16(REG_CALIBRATION, calib);
}

void ina226_calibrate() {
    current_lsb = cfg.max_current / 32768.0;
    uint16_t calib = (uint16_t)(0.00512 / (current_lsb * cfg.shunt_ohms));
...
int detect_model() {
    int mfg = read16(REG_MANUFACTURER);
    printf("model: 0x%04X\n", mfg);
    printf("Detected Manufacturer ID: 0x%04X\n", read16(REG_MANUFACTURER));
...
void ina_read(float *voltage, float *current, float *power, float *shunt) {
    if (voltage) {
        uint16_t bus_raw = read16(REG_BUS_VOLTAGE);
        *voltage = ((bus_raw >> 3) * (cfg.model == MODEL_INA219?0.004:0.00125));
    }
    if (shunt) {
        int16_t raw = (int16_t)read16(REG_SHUNT_VOLTAGE);
    ...
    if (power) {
        uint16_t raw = read16(REG_POWER);
        *power = raw * power_lsb * 1000.0;
    }

}

void print_mqtt_reading(MQTTClient client) {
    float voltage = 0.0, shunt = 0.0;
    int16_t raw_current = read16(REG_CURRENT);
    int16_t raw_power = read16(REG_POWER);
    int16_t raw_shunt = read16(REG_SHUNT_VOLTAGE);
    uint16_t raw_bus = read16(REG_BUS_VOLTAGE);
...
int main(int argc, char *argv[]) {
    ...
    if (fd < 0) { perror("I2C Setup Failed"); return 1; }
